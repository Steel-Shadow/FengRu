...
float my_buffer[1800]; //模型单词输入最多为128 float，冗余设置
...
constexpr int kTensorArenaSize = 60 * 1024; //初始值为 30*1024  60有第一条失败 80无第一条失败 100失败 向下调整 20失败
...
void loop() {
  ...
  if (done_just_triggered) {
    ...
    for (size_t i = 0; i < 100; ++i) { //模型输入
      model_input->data.f[i] = acceleration_data[i];
    }

    TfLiteStatus invoke_status = interpreter->Invoke(); //模型运行
