...
constexpr int stroke_transmit_max_length = 300;  // 原为160
...
void SetupIMU() {
    ...
    // If you see an error on this line, make sure you have at least v1.1.0 of
    // the Arduino_LSM9DS1 library installed.
    ...
    MicroPrintf("Gyroscope sample rate %d.%d Hz",
                static_cast<int32_t>(rate_int),
                static_cast<int32_t>(rate_frac * 100));
...
void EstimateGyroscopeDrift(float* drift) {
    const bool isMoving =
        VectorMagnitude(current_velocity) > 0.005f;  // 原为0.1
...
bool IsMoving(int samples_before) {
    constexpr float moving_threshold = 0.5f;  // 原为10.0
...
void UpdateStroke(int new_samples, bool* done_just_triggered) {
    constexpr int minimum_stroke_length = moving_sample_count + 300;  // 原为+10
    constexpr float minimum_stroke_size = 0.1f;  // 原为0.2
            ...
            if (!is_moving) {
                if (stroke_length > minimum_stroke_length) {
                    *stroke_state = eDone;
                    ...
                    // stroke_length = 0; // 原为 stroke_length = 0
                    *stroke_state = eDrawing;  // 原为*stroke_state = eWaiting
#ifdef MAGIC_WAND_DEBUG
...
void getScore(int8_t score,  // TODO
              uint8_t* score_int_real,
              uint8_t* score_frac_real,
              TfLiteTensor* output) {
    float score_f = (score - output->params.zero_point) * output->params.scale;
    float score_int;
    float score_frac = modf(score_f * 100, &score_int);
    *score_int_real = static_cast<char>(score_int);
    *score_frac_real = static_cast<char>(score_frac * 100);
}
}  // namespace
...
void setup() {
    ...
    service.addCharacteristic(ansChar);
    ...
    if ((model_input->dims->size != 4) || (model_input->dims->data[0] != 1) ||
        ...
        MicroPrintf("%d %d %d %d %d %d %d %f ", model_input->dims->size,
...
void loop() {
    ...
    if (gyroscope_samples_read > 0) {
        ...
        //     ansChar.writeValue(stroke_struct_buffer, label_count);
        // }
    }
    ...
    if (done_just_triggered) {
        ...
        int8_t max_score;
        int max_index;
        uint8_t score_int_array[label_count] = {0};
        uint8_t score_frac_array[label_count] = {0};
        for (int i = 0; i < label_count; ++i) {  // label_count
            const int8_t score = output->data.int8[i];

            getScore(score, &score_int_array[i], &score_frac_array[i], output);
            MicroPrintf(
                "[%d]: score is %d, score_int is %d, score_frac is %d\n", i,
                score, score_int_array[i], score_frac_array[i]);
        ...
        if (central && central.connected()) {
            ansChar.writeValue(score_int_array, label_count);
        }

        uint8_t *max_score_int_real, *max_score_frac_real;
        getScore(max_score, max_score_int_real, max_score_frac_real, output);
        MicroPrintf("Found %s: %d (%d.%d)", labels[max_index], max_score,
                    *max_score_int_real, *max_score_frac_real);
    }
}
