...
constexpr int stroke_transmit_max_length = 200; //原为160
...
void EstimateGyroscopeDrift(float* drift) {
  const bool isMoving = VectorMagnitude(current_velocity) > 0.005f; //原为0.1
  if (isMoving) {
    return;
  }
      ...
      if (!is_moving) {
        if (stroke_length > minimum_stroke_length) {
          *stroke_state = eDone;
          ...
          // stroke_length = 0; //TODO 原为 stroke_length = 0
          *stroke_state = eDrawing; //TODO 原为*stroke_state = eWaiting
        }
      }
    }
